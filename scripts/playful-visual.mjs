...
const evidencePrefix = process.env.EVIDENCE_PREFIX ?? "playful";
const browser = await chromium.launch();
const results = [];
for (const width of [320, 390, 768, 1440]) {
  for (const [scene, role, path, label = scene] of [
    ["venue-quote", "customer", "/", "home"],
    ["venue-quote", "customer", "/venues", "catalog"],
    ...
    if (width === 390 || width === 1440)
      await page.screenshot({
        path: `docs/evidence/${evidencePrefix}-${label}-${width}.png`,
        fullPage: true,
      });
    results.push({ width, scene, label, path, errors, ...metrics });
    if (path === "/venues/sample-pavilion" && width === 390) {
      await page
        .getByRole("button", { name: "02 Seated gathering", exact: true })
        .click();
      await page.screenshot({
        path: `docs/evidence/${evidencePrefix}-gallery-390.png`,
        fullPage: true,
      });
    }
    await page.close();
...
await browser.close();
writeFileSync(
  `docs/evidence/${evidencePrefix}-visual.json`,
  JSON.stringify(
